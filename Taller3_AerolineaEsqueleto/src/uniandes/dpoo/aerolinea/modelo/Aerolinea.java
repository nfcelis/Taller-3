...
public class Aerolinea
{
    ...
    public Vuelo getVuelo( String codigoRuta, String fechaVuelo )
    {
    	...
    	int i=0;
    	while (encontrado==null && i<vuelos.size()) {
    ...
    public Collection<Tiquete> getTiquetes( )
    {
    	...
    	for(Vuelo vuelo: vuelos) {
    		...
    		tiquetes.addAll(tiquetes_vuelo);
    	}
    ...
    public void programarVuelo( String fecha, String codigoRuta, String nombreAvion ) throws Exception
    {
        ...
        Avion avion = getAviones().stream()
        	    .filter(a -> a.getNombre().equals(nombreAvion))
        	    .findFirst()
        	    .orElseThrow(() -> new Exception("El avión con el nombre " + nombreAvion + " no está registrado."));
      
        for (Vuelo vuelo : getVuelos()) {
            if (vuelo.getFecha().equals(fecha) && vuelo.getAvion().equals(avion)) {
                throw new Exception("El avión " + nombreAvion + " ya tiene un vuelo programado en esa fecha.");
            }
        }
    ...
    public int venderTiquetes( String identificadorCliente, String fecha, String codigoRuta, int cantidad ) throws VueloSobrevendidoException, Exception
    {
        ...
        if (vuelo == null) {
            throw new Exception("Vuelo no encontrado para la ruta y fecha proporcionadas.");
        }
        Collection<Tiquete> tiquetes;
        ...
        CalculadoraTarifasTemporadaAlta CalculadoraAlta=new CalculadoraTarifasTemporadaAlta();;
        CalculadoraTarifasTemporadaBaja CalculadoraBaja=new CalculadoraTarifasTemporadaBaja();;
        
		int precio_total= vuelo.venderTiquetes(cliente, CalculadoraAlta, CalculadoraBaja, cantidad);
